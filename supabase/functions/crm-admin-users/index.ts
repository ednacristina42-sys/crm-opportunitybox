...
// Só chamável por Admin (Edna/Paulo). Nunca expõe a service_role key ao
// browser — fica só dentro desta função, exactamente como em crm-toconline.
// Pedido pela Edna, 23/09/2026: conta "administrativo" (Carolina Martins)
// e futuras contas semelhantes.
// ══════════════════════════════════════════════════════════════════════════
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const rPerfil = await fetch(`${cfg.url}/rest/v1/ob_profiles`, {
      ...
      body: JSON.stringify({ id: novoId, full_name: fullName, email, role, active: true }),
    });
    if (!rPerfil.ok) {
